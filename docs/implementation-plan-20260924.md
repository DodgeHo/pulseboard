...
## 2026-09-25 Project Copy And Multilingual Layout Pass

- Start from the current clean worktree and re-read the portal content, build scripts, runtime language switcher, styles, public-surface verifiers, and deployment notes before editing. Keep the existing Signal Lattice / Composition C visual identity intact.
- Add a small localized auxiliary-name data shape for featured projects so English names, Chinese project names, and Japanese auxiliary names can be rendered consistently on the homepage, `/projects/`, localized home routes, localized project archive routes, and project detail pages without hard-coding one locale into another.
- Rewrite the ten requested project summaries in English, Traditional Chinese, Simplified Chinese, and Japanese from conservative confirmed facts only. Synchronize `project-overrides.mjs`, build-time homepage definitions, and the runtime `main.js` locale descriptions so language switching cannot restore old copy.
- Treat Chinese project names as real secondary headings rather than tiny metadata: increase their visual weight, preserve line wrapping, add `min-width: 0` where card grids and title rows need it, and keep long summaries and action buttons from overlapping or escaping their rows.
- Preserve safety boundaries for closed-source/private items: PuzzleWear links only to the public `/login` entry, CWC remains a security-reviewed public description with no customer/data/infrastructure/internal implementation leakage, and private GitHub URLs, clone URLs, API URLs, tokens, cookies, keys, and credentials never appear in generated output.
- Update verifiers for the PuzzleWear `/login` URL, required localized auxiliary names, summary rewrites, removed old phrases, ICP locale boundary, practice-project presence, and private-link leakage while keeping existing inventory-count and public-link checks.
- Verify with the project scripts: `corepack pnpm build:portal`, `corepack pnpm verify:portal`, `corepack pnpm verify:public:local`, `node --check apps/portal/src/main.js`, `git diff --check`, and Impeccable detector over the portal UI files.
- Run bounded browser checks at 1440px, 768px, and 390px or 360px for `/`, `/projects/`, `/zh-hans/`, `/zh-hans/projects/`, `/zh-hant/`, `/ja/`, and representative project detail pages including HeatStack, TapPhysics, PulseBoard, PuzzleWear, SAA, SAP, and ISPM. Check no horizontal overflow, readable focus states, reduced-motion behavior, card/button wrapping, and locale-specific ICP behavior.
- Deploy only after local build, static verification, and browser inspection pass. Use only the existing Tencent Cloud Ubuntu + Docker Compose + Nginx workflow if credentials and scripts are already available; do not create resources, change DNS, change databases, generate keys, or print secrets. If deployment credentials are unavailable, stop after local verification and report that external deployment was not executed.
- Rollback remains targeted and non-destructive: review `git diff` and revert only this round’s hunks with a focused patch if needed, never with `git reset --hard` or broad checkout commands.
