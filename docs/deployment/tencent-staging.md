...
The workflow builds and verifies the project portal and namespaced PulseBoard artifacts on the GitHub runner, resolves that checkout to an immutable commit SHA, refuses to deploy over a dirty server worktree by default, and checks out the same commit on the host. It stops rather than deploying when the requested branch or tag moves between those two checkouts. The host checkout is the deployment source for the static portal, PulseBoard HTML, and reviewed Nginx configuration; the workflow does not transfer the full generated portal archive over the GitHub runner's SSH connection. It rebuilds the production compose stack, verifies `127.0.0.1:4000` health endpoints, runs `pnpm demo:flow` inside the API container, backs up and installs the portal and `/demo/` artifacts, installs the Nginx contract to both `sites-available/anlan.conf` and the staging host's regular `sites-enabled/anlan.conf`, reloads Nginx after `nginx -t`, and finally runs `pnpm verify:public` against the configured public base URL. The Nginx contract preserves the independent Career Radar proxy at `/jobs/`. If a reviewed deployment must proceed over a dirty staging checkout, explicitly enable `preserve_dirty_worktree`; the workflow records the current HEAD and status, exports tracked patches, archives untracked files, and creates a Git stash under a timestamped `$HOME/pulseboard-deploy-backups/` record before checkout. It does not create DNS, TLS, Tencent Cloud, or AWS resources; those must be approved and prepared separately.

One-shot Compose commands in the SSH heredoc must keep stdin detached with `-T` and `</dev/null`. Otherwise `docker compose run` can consume the rest of the deployment script, report a successful migration, and silently skip application recreation, demo verification, static installation, and Nginx reload. The workflow writes a release-specific completion marker only after all remote checks pass, verifies that marker in a second SSH command, and requires the public liveness response to report the exact checked-out SHA.
